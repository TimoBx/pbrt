...
void TimelineWidget::addCurve(NodeWidget *widget,const QStringList &varName,Curve *curve,const QColor c,float minVal,float maxVal) {
  const unsigned int index = _params.size();
  ...
  for(unsigned int i=0;i<curve->nbTangents();++i) {
    _tangents[index].push_back(new TangentPoint(this,_params[index],i));
    _scene->addItem(_tangents[index][i]);
  }

  _points.push_back(vector<ControlPoint *>());
  for(unsigned int i=0;i<curve->nbPoints();++i) {
    TangentPoint *t = _tangents[index].size()>i ? _tangents[index][i] : NULL;
    _points[index].push_back(new ControlPoint(this,_params[index],i,t));
    _scene->addItem(_points[index][i]);
  }

  _tlines.push_back(vector<TangentLine *>());
  for(unsigned int i=0;i<min(curve->nbTangents(),curve->nbPoints());++i) {
    _tlines[index].push_back(new TangentLine(_params[index],_points[index][i],_tangents[index][i]));
    _scene->addItem(_tlines[index][i]);
  }
...
void TimelineWidget::controlPointsChanged(const QStringList &varName) {
  for(unsigned int i=0;i<_params.size();++i) {
	...
	_tangents[i].push_back(new TangentPoint(this,_params[i],j));
	_scene->addItem(_tangents[i][_tangents[i].size()-1]);
      }
      
      for(unsigned int j=0;j<_params[i]->curve->nbPoints();++j) {
	TangentPoint *t = _tangents[i].size()>j ? _tangents[i][j] : NULL;
	_points[i].push_back(new ControlPoint(this,_params[i],j,t));
	...
	_tlines[i].push_back(new TangentLine(_params[i],_points[i][j],_tangents[i][j]));
