...
public:
  ...
  NodeInterface(PbGraph *parent=NULL,NodeHandle *handle=NULL);
 ...
 public:
  ...
  virtual const QString     desc();
  virtual const QString     help();
  ...
  inline void setHandle(NodeHandle *handle) {_handle=handle;}
  inline NodeHandle *handle() {return _handle;}
 ...
 // init all the output data and set the _ready variable (inputs should be ok)
  ...
  inline virtual const Vector2f outputContentSize(unsigned int i) {return outputSize(i);}
  ...
  virtual void exportOutputContent(const QString &,unsigned int) {}
 ...
 private:
  ...
  inline PbGraph *parent() const {return _parent;}
  inline void setParent(PbGraph *parent) {_parent=parent;}
  
  // state of the node (to remember if the node needs to be updated)
  inline virtual int state() const {return _state;}

 public:
  inline virtual bool isGrouped() const {return false;}
 ...
 protected:
  ...
  //static FloatTexture2D *_nodeUnselected;
  //static FloatTexture2D *_nodeSelected;
  //static FloatTexture2D *_outputUnselected;
  //static FloatTexture2D *_outputSelected;
  static TextDrawer     *_textDrawer;
...
class LIBRARY_EXPORT DefaultNode : public NodeInterface {
 public:
  DefaultNode(PbGraph *parent,unsigned int nbInputs,unsigned int nbOutputs)
    : NodeInterface(parent),_nbInputs(nbInputs),_nbOutputs(nbOutputs) {}
  
  inline virtual unsigned int nbInputs () const {return _nbInputs; }
  inline virtual unsigned int nbOutputs() const {return _nbOutputs;}
...
inline void NodeInterface::draw(const Matrix4f &mvp) {
  ...
  std::vector<Vector2f> pointsIn(nbInputs());
  std::vector<Vector2f> pointsOut(nbOutputs());
  std::vector<Vector2f> boxesPts(nbOutputs()*6+6);
  std::vector<Vector2f> boxesCos(nbOutputs()*6+6);

  boxesPts[0] = _pos;
  boxesPts[1] = _pos+Vector2f(_size[0],0);
  boxesPts[2] = _pos+_size;
  boxesPts[3] = _pos+_size;
  boxesPts[4] = _pos+Vector2f(0,_size[1]);
  boxesPts[5] = _pos;
  ...
  for(unsigned int i=0;i<nbInputs();++i) {
    pointsIn[i] = inputNodeCoord(i);
  }

  for(unsigned int i=0;i<nbOutputs();++i) {
    const Vector2f tmpPos = _pos+outputPos(i);
    pointsOut[i]   = outputNodeCoord(i);
    boxesPts[k++]  = tmpPos;
    boxesPts[k++]  = tmpPos+Vector2f(outputSize(i)[0],0);
    boxesPts[k++]  = tmpPos+outputSize(i);
    boxesPts[k++]  = tmpPos+outputSize(i);
    boxesPts[k++]  = tmpPos+Vector2f(0,outputSize(i)[1]);
    boxesPts[k++]  = tmpPos;
    boxesCos[l++]  = Vector2f(0.0f,0.0f);
    boxesCos[l++]  = Vector2f(1.0f,0.0f);
    boxesCos[l++]  = Vector2f(1.0f,1.0f);
    boxesCos[l++]  = Vector2f(1.0f,1.0f);
    boxesCos[l++]  = Vector2f(0.0f,1.0f);
    boxesCos[l++]  = Vector2f(0.0f,0.0f);
  }
  ...
  glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,0,pointsIn[0].data());
  glDrawArrays(GL_POINTS,0,nbInputs());

  // outputs 
  _pconnect->setUniform3fv("color",(GLfloat *)pOut.data());
  glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,0,pointsOut[0].data());
  glDrawArrays(GL_POINTS,0,nbOutputs());
  _pconnect->disable();
  ...
  glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,0,boxesPts[0].data());
  glVertexAttribPointer(1,2,GL_FLOAT,GL_FALSE,0,boxesCos[0].data());
  ...
  // output text
  const QStringList lt = outputDesc();
...
inline void NodeInterface::receiveEvent(const NodeEvent &event) {
  if(event.type()==NodeEvent::FRAME_HAS_CHANGED && widget()) {
