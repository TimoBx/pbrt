...
#define ERROR(msg) { QMessageBox::information(NULL,tr("Error"),msg); }
...
bool GenericCustomWidgetSetting::checkParams() {
  ...
  if(!ok) {
    ERROR("Output size must contain valid numbers.");
    return false;
  }

  if((inputItems.empty() || ts==Vector2f(0,0)) && to==Vector2f(0,0)) {
    ERROR("Output size must not be zero");
    return false;
  }

  if(outputItems.empty()) {
    ERROR("At least one output image needed");
    return false;
  }
  ...
  if(names.removeDuplicates()>0) {
    ERROR("At least two parameters/inputs/outputs have the same name");
    return false;
  }
  ...
  for(int i=0;i<names.size();++i) {
    ...
    if(names[i].contains(' ')) {
      ERROR("Names must not contain spaces");
      return false;
    }
  }

  if(outputItems.empty()) {
    ERROR("The node should contain at least one output");
    return false;
  }
