...
PoissonGreyNode::PoissonGreyNode(PbGraph *parent,NodeHandle *handle)
  : NodeTexture2D(parent,handle),
    ...
    _debugMode(SOLUTION),
    _debugLevel(0),
    _debugProgram(QString(plugDir()+"/textureNodes/poissonGrey/shaders/psbw.vert").toStdString(),
		  QString(plugDir()+"/textureNodes/poissonGrey/test.frag").toStdString()) {
...
const QString PoissonGreyHandle::name() {
  return "imgPoissonGrey";
}

const QString PoissonGreyHandle::path() {
  return "diffusion/";
}

const QString PoissonGreyHandle::desc() {
  return "Poisson reconstruction with Dirichlet constraints";
}

const QString PoissonGreyHandle::help() {
  return tr("Poisson reconstruction<br>"
	    "This node is waiting for (gx,gy,-,d)<br>"
	    "where gx, gy is the input gradient and <br>"
	    "d is a Dirichlet constraint<br>"
	    "The Dirichlet constraint won't be taken into account if d<0");
}

const QStringList PoissonGreyHandle::inputDesc() {
  return (QStringList() << "gradient");
}

const QStringList PoissonGreyHandle::outputDesc() {
  return (QStringList() << "diffused");
}

NodeInterface *PoissonGreyHandle::createInstance(PbGraph *parent,unsigned int,unsigned int) {
  return new PoissonGreyNode(parent,this);
}

QT_BEGIN_NAMESPACE
Q_EXPORT_PLUGIN2(helloEx,PoissonGreyHandle)
QT_END_NAMESPACE
