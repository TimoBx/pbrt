...
bool NodeManager::saveCustomNode(CustomNodeHandle *handle,NodeInterface *node) {
  ...
  //cout << __FILE__ << " - " << __FUNCTION__ << " -> saving " << handle->coreStringId().toStdString() << endl;
...
NodeInterface *NodeManager::loadNode(SceneLoader *loader,PbGraph *parent,bool &ret,unsigned int id) {
  ...
  //cout << __FILE__ << " - " << __FUNCTION__ << " - " << __LINE__ << " - " << nodeStringId.toStdString() << endl;
...
NodeHandleInterface *NodeManager::createCustomNode(NodeInterface *node,const QString &stringId,unsigned int version,
  ...
  const QString origStringId = isGroupedNode(node) ? node->stringId() : coreStringId(node->name(),node->path());
  const unsigned int origVersion = isGroupedNode(node) ? node->version() : coreVersion(node->name(),node->path());
  ...
  i = _customHandles.find(nodeStringIdVersion(stringId,version));
  if(i!=_customHandles.end()) {
    alreadyExist = true; 
  } else {
  ...
  if(alreadyExist) {
    ...
    // create a new handle 
    chandle = new CustomNodeHandle(filename,origStringId,origVersion,stringId,version,
  ...
  if(isGroupedNode(node)) {
    ...
    _customHandles[nodeStringIdVersion(stringId,version)] = chandle;
  }
  
  _idConvert[nodePathName(path,name)] = nodeStringIdVersion(stringId,version);
...
const QString NodeManager::coreStringId(const QString &name,const QString &path) {
...
unsigned int NodeManager::coreVersion(const QString &name,const QString &path) {
  if(isCoreNode(name,path)) {
