...
void GraphWidget::render() {
  ...
  const Matrix4f mvp = Glutils::screenMode(devicePixelRatio()*width(),devicePixelRatio()*height(),_offset*devicePixelRatio(),_scale);
...
void GraphWidget::mouseMoveEvent(QMouseEvent *me) {
  ...
  if(_interaction==CONNECT_MOTION) {
    ...
    moveSelection(Vector2f(me->x()-_mouseTmp[0],(height()-me->y())-_mouseTmp[1])*_scale*devicePixelRatio());
    _mouseTmp = Vector2f(me->x(),height()-me->y());
