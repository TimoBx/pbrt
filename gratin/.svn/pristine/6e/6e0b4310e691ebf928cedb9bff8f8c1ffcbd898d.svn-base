...
class GridCustomWidget : public GenericCustomWidget {
  ...
  inline IntSliderWidget *gridDepth() {return _gridDepth;}
...
class GridCustomNode : public GenericCustomNode {
 public:
  GridCustomNode(PbGraph *parent=NULL,NodeHandle *handle=NULL);
  ~GridCustomNode();
...
class GridCustomHandle : public QObject, public NodeHandleInterface {
  Q_OBJECT
  Q_INTERFACES(NodeHandleInterface)
  Q_PLUGIN_METADATA(IID "Gratin.GridCustom")

 public:
  const QString     stringId() const {return "gridGenericId";}
  unsigned int      version () const {return 1;}

  const QString     name    () const {return "gridGeneric";}
  const QString     path    () const {return "generic/";}
  const QString     desc    () const {return "Generic node for 3D grid processing";}
  ...
  NodeInterface *createInstance(PbGraph *parent) {
    return new GridCustomNode(parent,new NodeHandle(this));
  }
  ...
  NodeInterface *createInstance(PbGraph *parent,
    ...
    return new GridCustomNode(parent,new NodeHandle(stringId(),version(),name,path,desc,help,
