...
//GlInteractionWidget::GlInteractionWidget(QGLContext *context,QGLWidget *share)
...
void GlInteractionWidget::paintGL() {
  if(isExposed()) {

    _glcontext->makeCurrent(this);
    
    const Vector4f background(0.78f,0.78f,0.78f,1.0f);
...
void GlInteractionWidget::initializeGL() {
  ...
  //_glf->glDisable(GL_COLOR_MATERIAL);
  //_glf->glEnable(GL_TEXTURE_2D);
  _glf->glDepthMask(GL_FALSE);
  _glf->glClear(GL_COLOR_BUFFER_BIT);
...
void GlInteractionWidget::exposeEvent(QExposeEvent *ev) {
  ...
  if(isExposed()) {
    ...
    if(_glConnected) {
      disconnect(this,&GlInteractionWidget::widthChanged,this,&GlInteractionWidget::resizeGL);
      disconnect(this,&GlInteractionWidget::heightChanged,this,&GlInteractionWidget::resizeGL);
      _glConnected = false;
    }
  }
}
