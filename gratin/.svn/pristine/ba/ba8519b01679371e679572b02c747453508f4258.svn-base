...
class PingPongCustomWidget : public GenericCustomWidget {
  ...
  inline IntSpinWidget *pingPongNumber() {return _pingPongNumber;}
  inline void setPingPongNumberLabel(unsigned int n) {
    _currentNumber->setText("Current pass: "+QString::number(n));
  }
 ...
 private:
  ...
  IntSpinWidget *_pingPongNumber;
  QLabel        *_currentNumber;
};
...
class PingPongCustomHandle : public QObject, public NodeHandleInterface {
 ...
 public:
  ...
  const QString     name    () const {return "pingPongGeneric";}
  const QString     path    () const {return "generic/";}
  const QString     desc    () const {return "Generic node for ping-pong processing";}
