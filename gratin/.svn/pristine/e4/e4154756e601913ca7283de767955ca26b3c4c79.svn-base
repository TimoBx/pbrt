...
const QString MainWindow::_gratinExt   = ".gratin";
const QString MainWindow::_defaultName = "untitled";
const QString MainWindow::_version     = "1.0";
QDir MainWindow::_currentPath = QDir("data/");

MainWindow::MainWindow()
  : QMainWindow() {

  // the opengl format is defined here
   QGLFormat fmt;
   fmt.setAlpha(true);
   fmt.setDepth(true);
   fmt.setRgba(true);
   ...
   _graphWidget         = new GraphWidget(fmt);
   _interactionWidget   = new GlInteractionWidget(new QGLContext(_graphWidget->format()),_graphWidget);
   ...
   _interlogWidget = new QDockWidget();
   QVBoxLayout *l = new QVBoxLayout();
   l->setContentsMargins(0,0,0,0);
   l->addWidget(_interactionWidget);
   l->addWidget(_logWidget);
   QWidget *tmpWidget = new QWidget();
   tmpWidget->setLayout(l);
   _interlogWidget->setWidget(tmpWidget);

   setCentralWidget(_graphWidget);
   ...
   LogWidget::instance()->setText("Welcome to "+Tags::rootTag()+" "+Tags::attribVersion()+" "+_version+" -- have fun!");
}
...
void MainWindow::createActions() {
  ...
  _nextFrameAct->setShortcut(QKeySequence(Qt::SHIFT + Qt::Key_Right));
  ...
  _prevFrameAct->setShortcut(QKeySequence(Qt::SHIFT + Qt::Key_Left));
  ...
  _firstFrameAct->setShortcut(QKeySequence(Qt::SHIFT + Qt::Key_Up));
  ...
  _lastFrameAct->setShortcut(QKeySequence(Qt::SHIFT + Qt::Key_Down));
  ...
  connect(_groupAct,SIGNAL(triggered()),this,SLOT(graphToGremlin()));
  ...
  connect(_ungroupAct,SIGNAL(triggered()),this,SLOT(gremlinToGraph()));
...
void MainWindow::open() {
  if(maybeSave()) {
    QString fileName = QFileDialog::getOpenFileName(this,tr("Open Gratin File"),_currentPath.absolutePath(),
						    QString("Gratin Files (*")+_gratinExt+QString(")"));
    if(!fileName.isEmpty()) {
      clearAll();
      loadFile(fileName);
...
bool MainWindow::saveAs() {
  ...
  if(!fileName.endsWith(_gratinExt)) 
    fileName.append(_gratinExt);
...
void MainWindow::about() {
  QString h = tr("<center><font size='12'> Gratin version 1.0</font></center><br>"
		 "A node-based compositing software for 2D and 3D animations.<br><br>"
		 "Copyright (C) 2013 "
		 "<a href='http://romain.vergne.free.fr'>Romain Vergne</a><br>"
		 "( <a href='mailto:romain.vergne@inria.fr'>romain.vergne@inria.fr</a> )<br><br>"
...
void MainWindow::graphToGremlin() {
  _graphWidget->graphToGremlin();
}

void MainWindow::gremlinToGraph() {
  _graphWidget->gremlinToGraph();
}
...
void MainWindow::setCurrentFile(const QString &fileName) {
  ...
  if(_currentFile.isEmpty())
    shownName = _defaultName+_gratinExt;
...
bool MainWindow::loadFile(const QString &fileName) {
  ...
  if(_loader.readFrom(file) && _loader.init(_version) && _loader.loadIOData("mainWindow",this)) {
...
void MainWindow::initHelpWidget() {
  vector<NodeHandle *> h = NodeManager::instance()->nodeHandles();
