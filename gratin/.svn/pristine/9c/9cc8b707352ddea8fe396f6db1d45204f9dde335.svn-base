...
bool PbGraph::makeConnection(NodeInterface *src, IONodeID outputId, NodeInterface *dst, IONodeID inputId) {
  ...
  // if(NodeManager::instance()->isGroupedNode(dst)) {
  //   NodeGrouped *n = (NodeGrouped *)dst;
  //   cout << __FILE__ << " - " << __FUNCTION__ << " --> connection done. Node ready ? " << n->isReady() << endl;
  //   for(unsigned int i=0;i<n->nbNodes();++i) {
  //     cout << "node " << i << " ---> " << n->node(i)->name().toStdString() << ", " << n->node(i)->isReady() << endl;
  //   }
  // }
...
void PbGraph::prepareForProcessing(bool needGraphIds,bool needTopList,bool updateNodes) {
  ...
  for(NodeID i=0;i<nbNodes();++i) {
    ...
    // cout << _topologicalList[i]->name().toStdString() << " -- " << _topologicalList[i]->nbInputs() << " -- " << _topologicalList[i]->nbOutputs() << endl;

    // this array is then given to the node
    _topologicalList[i]->setInputData(inConnections);
    //cout << __FILE__ << " - " << __LINE__ << " - " << this << endl;
    if(updateNodes) {
      //cout << __FILE__ << " - " << __LINE__ << " - " << this << endl;
...
// NodeGrouped *PbGraph::connexPartToNodeGrouped(NodeInterface *node) {
...
// NodeGrouped *PbGraph::nodeGroupedToConnexPart(NodeGrouped *nodeGrouped) {
