...
GPUProgram::GPUProgram(GPUShader* vs,GPUShader* fs,GPUShader *gs,GPUShader *tcs,GPUShader *tes)
  : _vs(vs),
    _fs(fs),
    _gs(gs),
    _tcs(tcs),
...
GPUProgram::GPUProgram(const QString &vsFile,
  ...
  if(tcsFile!="")
    _tcs = new GPUShader(TESSC,tcsFile,true,fromFile);

  if(tesFile!="")
    _tes = new GPUShader(TESSE,tesFile,true,fromFile);
