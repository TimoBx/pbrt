...
CurveInterpolator::CurveInterpolator(CurveInterpolator *curve) {
  ...
  _tangents = std::vector<Vector2f>(curve->nbTangents());
...
void CurveInterpolator::setTangent(const Vector2f &pt,unsigned int i) {
...
void HermiteInterpolator::addKeyframe(const Vector2f &pt) {
  ...
  if(nb!=nbPoints()) {
    for(unsigned int i=0;i<nbPoints();++i) {
      if(_points[i]==pt) {
	_tangents.insert(_tangents.begin()+i,Vector2f(1,0));
	break;
      }
    }
  }
...
void HermiteInterpolator::setTangent(const Vector2f &pt,unsigned int i) {
  const Vector2f t = pt.normalized();
  CurveInterpolator::setTangent(t,i);
}
...
void HermiteInterpolator::initTangents() {
  ...
  for(int i=0;i<nb;++i) {
    _tangents.push_back(Vector2f(1,0));
  }
...
inline bool CurveInterpolator::save(SceneSaver *saver) {
  ...
  for(unsigned int i=0;i<nbTangents();++i) {
    if(!saver->saveVector2f(Tags::attribId("controlTangent",i),_tangents[i])) 
      ret = false;
  }
...
inline bool CurveInterpolator::load(SceneLoader *loader) {
  ...
  for(unsigned int i=0;i<nbt;++i) {
    if(!loader->loadVector2f(Tags::attribId("controlTangent",i),_tangents[i])) 
      ret = false;
  }
...
// TODO: change it -> call curve interpolator and curve behavior in the good range
  vector<Vector2f> pts;
