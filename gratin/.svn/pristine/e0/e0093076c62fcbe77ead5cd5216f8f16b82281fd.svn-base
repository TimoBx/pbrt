...
#ifndef IMGSPRITESCUSTOM_H
#define IMGSPRITESCUSTOM_H
...
class ImgSpritesCustomNode;

class ImgSpritesCustomWidget : public GenericCustomWidget {
  Q_OBJECT
    
    public:
  ImgSpritesCustomWidget(ImgSpritesCustomNode *node);
};


class ImgSpritesCustomNode : public GenericCustomNode {
 public:
  ImgSpritesCustomNode(PbGraph *parent=NULL,NodeHandle *handle=NULL);
  ~ImgSpritesCustomNode();
...
class ImgSpritesCustomHandle : public QObject, public NodeHandleInterface {
  Q_OBJECT
  Q_INTERFACES(NodeHandleInterface)
  Q_PLUGIN_METADATA(IID "Gratin.ImgSpritesCustom")

 public:
  const QString     stringId() const {return "imgSpritesGenericId";}
  unsigned int      version () const {return 1;}

  const QString     name    () const {return "imgSpritesGeneric";}
  const QString     path    () const {return "generic/";}
  const QString     desc    () const {return "Generic node for image sprites processing";}
  ...
  NodeInterface *createInstance(PbGraph *parent) {
    return new ImgSpritesCustomNode(parent,new NodeHandle(this));
  }
  ...
  NodeInterface *createInstance(PbGraph *parent,
    ...
    return new ImgSpritesCustomNode(parent,new NodeHandle(stringId(),version(),name,path,desc,help,
							  inputNames,outputNames));
  }
};


#endif // IMGSPRITESCUSTOM_H
