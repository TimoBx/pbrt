...
inline void PbGraph::applyAllWithCond(const NodeCondition &condition) {
  std::vector<bool> seen(nbNodes(),false);
  std::vector<bool> needUpdate(nbNodes(),false);

  // mark all nodes that satisfy the condition
  for(unsigned int i=0;i<nbNodes();++i) {
    if(_topologicalList[i]->satisfy(condition)) {
      needUpdate[i] = true;
    }
  }

  // mark all of their connected nodes
  for(unsigned int i=0;i<nbNodes();++i) {
    if(!seen[i] && needUpdate[i]) {
      seen[i] = true;
      for(unsigned int j=0;j<_perNodeTL[i].size();++j) {
	needUpdate[_perNodeTL[i][j]] = true;
	seen[_perNodeTL[i][j]] = true;
      }
    }
  }
