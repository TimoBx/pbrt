...
const QStringList TextureIO::askForMultipleLoadParams() {
  QString format = "Images (";
  for(int i=0;i<_formats.size();++i) {
    format = format + "*" + _formats[i];
    if(i<_formats.size()-1) {
  ...
  QStringList filenames = QFileDialog::getOpenFileNames(0,"Load Images",_currentPath.absolutePath(),format);
  ...
  QDir d(".");
  QStringList l = filenames[0].split("/");
  _currentPath = d.filePath(filenames[0]);
  _path = _currentPath.absolutePath();
  _name =  l[l.size()-1];
