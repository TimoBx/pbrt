...
void GlInteractionWidget::render() {
  ...
  const Matrix4f mvp = Glutils::screenMode(devicePixelRatio()*width(),devicePixelRatio()*height(),_offset,_scale);
...
void GlInteractionWidget::mouseMoveEvent(QMouseEvent *me) {
  ...
  if(_interaction==SCALING_MOTION) {
    ...
    _offset  -= Vector2f(me->x()-_mouseTmp[0],(height()-me->y())-_mouseTmp[1])*_scale*devicePixelRatio();
    _mouseTmp = Vector2f(me->x(),height()-me->y());
  }
