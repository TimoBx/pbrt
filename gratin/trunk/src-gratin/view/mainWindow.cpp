...
void MainWindow::open() {
  if(maybeSave()) {
    QSettings settings;
    QString fileName = settings.value("files/load", QDir::currentPath()).toString();
    fileName = QFileDialog::getOpenFileName(this,tr("Open Gratin File"),fileName,
                            QString("Gratin Files (*")+GRATIN_EXT+QString(")"));
    if(!fileName.isEmpty()) {
      clearAll();
      loadFile(fileName);

      settings.setValue("files/load", fileName);
    }
  }
}
...
bool MainWindow::saveAs() {
  QSettings settings;
  QString fileName = settings.value("files/save", QDir::currentPath()).toString();
  fileName = QFileDialog::getSaveFileName(this,tr("Save Gratin File"),fileName,
                          QString("Gratin Files (*")+GRATIN_EXT+QString(")"));
  if(fileName.isEmpty())
    return false;
  ...
  settings.setValue("files/save", fileName);
