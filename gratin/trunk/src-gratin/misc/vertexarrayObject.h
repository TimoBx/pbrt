...
class LIBRARY_EXPORT VertexarrayObject : public GlContext {
 ...
 VertexarrayObject()
  ...
  inline void drawElements(GLenum mode,GLsizei count,GLsizei offset=0);
...
inline void VertexarrayObject::drawElements(GLenum mode,GLsizei count,GLsizei offset) {
  assert(_indices!=NULL);
  _glf->glDrawElements(mode,count,_indexType,reinterpret_cast<GLvoid*>(offset));
}
