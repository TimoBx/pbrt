...
QString composeGlslVersion(int major, int minor){
    return QString::number(major)+QString::number(minor)+"0";
}
const QString defaultGlslVersion = composeGlslVersion(OPENGL_MAJOR_VERSION, OPENGL_MINOR_VERSION);
...
GenericCustomNode::GenericCustomNode(bool useVertex,bool useTesselation,bool useGeometry,bool useFragment,
  ...
  : NodeTexture2D(parent,handle),
    _glslVersion(defaultGlslVersion),
...
bool GenericCustomNode::load(SceneLoader *loader) {
  ...
  // detect improper glsl version
  if (_glslVersion != defaultGlslVersion)
      std::cerr << "[warning] OpenGL version in file ("
                << qPrintable(_glslVersion)
                << ") mismatches system version ("
                << qPrintable(defaultGlslVersion)
                << ")."
                << std::endl;
