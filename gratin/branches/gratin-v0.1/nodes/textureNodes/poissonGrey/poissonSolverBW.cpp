...
void PoissonSolverBW::initShaders() {
  // shader creation 
  _residual   = new GPUProgram(_bwPath+"psbw.vert",_bwPath+"psbw_residual.frag");
  _divergence = new GPUProgram(_bwPath+"psbw.vert",_bwPath+"psbw_divergence.frag");
...
PoissonLevelBW::PoissonLevelBW(const string &bwPath,int level,int w,int h,FloatTexture2D *output) 
  ...
  _restrict = new GPUProgram(_bwPath+"psbw.vert",_bwPath+"psbw_restrict.frag");
  _refine1  = new GPUProgram(_bwPath+"psbw.vert",_bwPath+"psbw_refine.frag");
  _refine2  = new GPUProgram(_bwPath+"psbw.vert",_bwPath+"psbw_refine.frag");
  ...
  if(level>0) {
    _interpolate = new GPUProgram(_bwPath+"psbw.vert",_bwPath+"psbw_interpolate0.frag");
    _interpolate->enable();
    _interpolate->addUniform("u");
    _interpolate->disable();

  } else {
    _interpolate = new GPUProgram(_bwPath+"psbw.vert",_bwPath+"psbw_interpolate1.frag");
    _interpolate->enable();
    _interpolate->addUniform("u");
