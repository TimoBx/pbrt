...
public:
  ...
  virtual void exportOutputContent(const QString &filename,unsigned int i);
...
inline void NodeTexture2D::drawOutputs(GLenum *buff,int nbOutputs,bool needBinding,bool needUnbinding) {
  ...
  glDrawBuffers(nbOutputs,buff);

  glEnableVertexAttribArray(0);
  glEnableVertexAttribArray(1);

  glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,0,_boxesPts[0].data());
  glVertexAttribPointer(1,2,GL_FLOAT,GL_FALSE,0,_boxesCos[0].data());
...
/* inline void NodeTexture2D::draw(const Matrix4f &mvp) { */
/*   NodeInterface::draw(mvp); */
/*   if(!isReady()) return; */

/*   const Vector2f o(_offset/2.0f); */
...
inline void NodeTexture2D::drawOutputContent(const Matrix4f &mvp,const Vector2f &position,unsigned int i) {  
  ...
  Vector2f boxesPts[6] = {position,position+Vector2f(t->w(),0),position+Vector2f(t->w(),t->h()),
			  position+Vector2f(t->w(),t->h()),position+Vector2f(0,t->h()),position};

  Vector2f boxesCos[6] = {Vector2f(0.0f,0.0f),Vector2f(1.0f,0.0f),Vector2f(1.0f,1.0f),
...
inline const Vector2f NodeTexture2D::outputContentSize(unsigned int i) {
  if(!isReady()) return NodeInterface::outputContentSize(i);
  FloatTexture2D *t = ((TextureData *)output(i))->tex();
