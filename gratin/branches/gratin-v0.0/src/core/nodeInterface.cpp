...
void NodeInterface::initGPUData() {
  _pconnect   = new GPUProgram(":/node-connect.vert",":/node-connect.frag");
...
void NodeInterface::setInputData(const vector<pair<NodeInterface *,Vector2u> > &dinputs) {
  ...
  const unsigned int inputDataSize = currentData.size();
...
bool NodeInterface::isPartlyConnected() {
  return parent() ? parent()->isNodePartlyConnected(this) : false;
}
...
const QString NodeInterface::name() {
  return _handle ? _handle->name() : QString("noname");
}

const QString NodeInterface::path() {
  return _handle ? _handle->path() : QString("nopath");
}

const QString NodeInterface::desc() {
  return _handle ? _handle->desc() : QString("nodesc");
}

const QString NodeInterface::help() {
  return _handle ? _handle->help() : QString("nohelp");
}

const QStringList NodeInterface::inputDesc() {
  return _handle ? _handle->inputDesc () : QStringList();
}

const QStringList NodeInterface::outputDesc() {
  return _handle ? _handle->outputDesc() : QStringList();
}
