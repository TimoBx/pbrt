...
public:
  ...
  // create a gremlin with the connex part of the graph associated with node.
  virtual Gremlin *connexPartToGremlin(NodeInterface *node); 

  // deploy the gremlin and put it in the graph (remove the node from the graph too).
  virtual Gremlin *gremlinToConnexPart(Gremlin *gremlin);
  ...
  inline unsigned int nbNodes() const {return _adjacencyMatrix.size(); }
