...
void GenericCustomWidget::applyClicked() {
...
void GenericCustomWidget::initFromSettings(const GenericCustomWidgetSetting &settings) {
  ...
  unsigned int nbIn  = iitems.size();
  unsigned int nbOut = oitems.size();
...
bool GenericCustomWidget::saveWidgets(SceneSaver  *saver) {
  ...
  if(!saver->saveUint("genericNbWidgets",_mapWidgets.size())) ret = false;
